...
public class UserController {
    ...
    public UserController(IUserService userService, ClientService clientService, IContactService contactService) {
        this.userService = userService;
        this.clientService = clientService;
        this.contactService = contactService;
    }
    ...
    public ModelAndView registeruser() {
        ...
        clientDTO.setUser(new UserDTO());     // importante para poder ingresar password
        clientDTO.setContact(new ContactDTO()); // también si usás contact.phone
    ...
    public ModelAndView register() {
        ...
        ClientDTO clientDTO = new ClientDTO();
        mv.addObject("client", clientDTO);
        clientDTO.setUser(new UserDTO());         // ¡Esto es crucial!
        clientDTO.setContact(new ContactDTO());   // Esto también
        // Asegurate de cargar y agregar las localidades al modelo
        System.out.println("[DEBUG] Localidades: " + localityService.getAll());
        mv.addObject("localities", localityService.getAll()); // 👈 ESTA LÍNEA ES CLAVE
        mv.setViewName(ViewRouteHelper.USER_REGISTER);
        System.out.println("getRegister");
        return mv;
    }
    ...
    public ModelAndView registeruser(@Valid @ModelAttribute("client") ClientDTO clientDTO,
        ...
        try {
            ...
            Contact contact = new Contact();
            contact.setPhone(clientDTO.getContact().getPhone());
            clientDTO.getUser().setEmail(clientDTO.getUser().getEmail());
